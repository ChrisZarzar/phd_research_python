...
EDITED: Chris Zarzar 17-Jul-2016
Made major change to program by first creating two rasters
from the single shapefile I created. I made a Depth raster
and a Velocity raster. Then I process the rasters just like
I do wiht the HEC-RAS rasters.

EDITED: Chris Zarzar 28-Sep-2017
Setting up the script to stop before the ensemble composites are created.
Still waiting for the HECRAS data from Michael. So I have to use the ensembleComposite
script to do that combination of the rasters I will wind up making.
...
#Path of model output CSV Files 
...
#Location of shapefile to raster converted files
preRasdir = path+"modelOutput\\shp2raster\\"
...
for csvFile in fileList:
    if csvFile.endswith('.csv'):
        ...
        """---CREATE DEPTH AND VELOCITY RASTERS FROM SHAPEFILE---"""
        ...
        #Create depth raster 
        outRasdepth= csvName+"_Depth.tif"
        
        print "Creating %s depth raster" % csvName
        #Convert the newly created shapefile to a water depth raster
        arcpy.PointToRaster_conversion(outPathshp+outFC, "Depth", preRasdir+outRasdepth, cellAssign, "", cellSize)


        #Create depth raster 
        outRasvelocity= csvName+"_Velocity.tif"
        
        print "Creating %s velocity raster" % csvName
        #Convert the newly created shapefile to a flow velocity raster
        arcpy.PointToRaster_conversion(outPathshp+outFC, "Velocity__", preRasdir+outRasvelocity, cellAssign, "", cellSize)

arcpy.Delete_management("tempLay") 


"""-----CREATE NEW iRIC RASTERS FROM CONDIITNOAL TESTINGS----"""

##List the files in the shapefile to raster directory
rasList = os.listdir(preRasdir)

##List the files found in dirList with their full pathname
fileList4 = [preRasdir+"\\"+filename4 for filename4 in rasList]

for inRas in fileList4:
    if inRas.endswith('.tif'):
        ...
        if "Depth" in rasName:
            ...
            print "Creating water depth rasters for "+rasName

            outRasfldext = rasName+"_fldext.tif"
            rasOut = Con(Raster(inRas) > 0.05,2,1)
            rasOut.save(outPathras+outRasfldext)

            outRaswd1 = rasName+"_wd_hlfft.tif"
            rasOut = Con(Raster(inRas) >= 0.15,2,1)
            rasOut.save(outPathras+outRaswd1)
            
            outRaswd2 = rasName+"_wd_1ft.tif"
            rasOut = Con(Raster(inRas) >= 0.305,2,1)
            rasOut.save(outPathras+outRaswd2)
            
            outRaswd3 = rasName+"_wd_2ft.tif"
            rasOut = Con(Raster(inRas) >= 0.61,2,1)
            rasOut.save(outPathras+outRaswd3)

            outRaswd4 = rasName+"_wd_3ft.tif"
            rasOut = Con(Raster(inRas) >= 0.91,2,1)
            rasOut.save(outPathras+outRaswd4)
            
            outRaswd5 = rasName+"_wd_4ft.tif"
            rasOut = Con(Raster(inRas) >= 1.22,2,1)
            rasOut.save(outPathras+outRaswd5)

            outRaswd6 = rasName+"_wd_5ft.tif"
            rasOut = Con(Raster(inRas) >= 1.5,2,1)
            rasOut.save(outPathras+outRaswd6)
            
            outRaswd7 = rasName+"_wd_6ft.tif"
            rasOut = Con(Raster(inRas) >= 1.83,2,1)
            rasOut.save(outPathras+outRaswd7)

        if "Velocity" in rasName:
            
            print "Creating flow velocity rasters for "+rasName
            
            outRasfv1 = rasName+"_fv_1mph.tif"
            rasOut = Con(Raster(inRas) >= 0.45,2,1)
            rasOut.save(outPathras+outRasfv1)

            outRasfv2 = rasName+"_fv_2mph.tif"
            rasOut = Con(Raster(inRas) >= 0.89,2,1)
            rasOut.save(outPathras+outRasfv2)
            
            outRasfv3 = rasName+"_fv_4mph.tif"
            rasOut = Con(Raster(inRas) >= 1.79,2,1)
            rasOut.save(outPathras+outRasfv3)
            
            outRasfv4 = rasName+"_fv_6mph.tif"
            rasOut = Con(Raster(inRas) >= 2.68,2,1)
            rasOut.save(outPathras+outRasfv4)

            outRasfv5 = rasName+"_fv_8mph.tif"
            rasOut = Con(Raster(inRas) >= 3.58,2,1)
            rasOut.save(outPathras+outRasfv5)
        ...
        #Now outside of these if statements do the critical conditions rasters so i can reference temp wd and fv rasters
            try:
                ...
                print "Creating critical condition rasters for "+rasName
                
                outRascrit1 = rasName+"_crit_hlf_4.tif"
                rasOut = Con((Raster(outPathras+outRaswd1)== 2) & (Raster(outPathras+outRasfv3)== 2),2,1)
                rasOut.save(outPathras+outRascrit1)
                
                outRascrit2 = rasName+"_crit_1_4.tif"
                rasOut = Con((Raster(outPathras+outRaswd2) == 2) & (Raster(outPathras+outRasfv3)== 2),2,1)
                rasOut.save(outPathras+outRascrit2)
                
                outRascrit3 = rasName+"_crit_2_4.tif"
                rasOut = Con((Raster(outPathras+outRaswd3) == 2) & (Raster(outPathras+outRasfv3)== 2),2,1)
                rasOut.save(outPathras+outRascrit3)

                outRascrit4 = rasName+"_crit_hlf_6.tif"
                rasOut = Con((Raster(outPathras+outRaswd1) == 2) & (Raster(outPathras+outRasfv4)== 2),2,1)
                rasOut.save(outPathras+outRascrit4)

                outRascrit5 = rasName+"_crit_1_6.tif"
                rasOut = Con((Raster(outPathras+outRaswd2) == 2) & (Raster(outPathras+outRasfv4)== 2),2,1)
                rasOut.save(outPathras+outRascrit5)

                outRascrit6 = rasName+"_crit_2_6.tif"
                rasOut = Con((Raster(outPathras+outRaswd3) == 2) & (Raster(outPathras+outRasfv4)== 2),2,1)
                rasOut.save(outPathras+outRascrit6)

                outRascrit7 = rasName+"_crit_3_2.tif"
                rasOut = Con((Raster(outPathras+outRaswd4) == 2) & (Raster(outPathras+outRasfv2)== 2),2,1)
                rasOut.save(outPathras+outRascrit7)
                
                outRascrit8 = rasName+"_crit_4_2.tif"
                rasOut = Con((Raster(outPathras+outRaswd5) == 2) & (Raster(outPathras+outRasfv2)== 2),2,1)
                rasOut.save(outPathras+outRascrit8)

                outRascrit9 = rasName+"_crit_5_1.tif"
                rasOut = Con((Raster(outPathras+outRaswd6) == 2) & (Raster(outPathras+outRasfv1)== 2),2,1)
                rasOut.save(outPathras+outRascrit9) 

                outRascrit10 = rasName+"_crit_6_1.tif"
                rasOut = Con((Raster(outPathras+outRaswd7) == 2) & (Raster(outPathras+outRasfv1)== 2),2,1)
                rasOut.save(outPathras+outRascrit10) 
...
##"""
##---------------------------------------------------------------------------
##START THE THIRD PART OF SCRIPT: CREATE COMPOSITE RASTERS
##---------------------------------------------------------------------------
##"""
##
